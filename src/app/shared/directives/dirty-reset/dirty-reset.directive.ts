...
import { OriginalValuesType } from './utils';
...
export class UIDirtyResetDirective {
  uiDisabledProperty = input<string>('uiDisabled');
  ...
  onClick(event: MouseEvent) {
    // Solo manejamos el reset si está habilitado y el formulario tiene cambios
    if (this.handleReset()) {
      this.resetForm();
      event.preventDefault(); // Evitar navegación si es un botón de tipo submit
  ...
  private isEqual(obj1: any, obj2: any): boolean {
    const normalizeValue = (value: any): any => {
      ...
      // Convertir números y strings numéricas al mismo tipo
      if (
        typeof value === 'number' ||
    ...
    console.log({ normalizedObj1 });
    console.log({ normalizedObj2 });

    console.log(
      JSON.stringify(normalizedObj1) === JSON.stringify(normalizedObj2)
    );
  ...
  resetForm(): void {
    if (this.form()) {
      ...
      setTimeout(() => {
        ...
        this.updateButtonState();
        this.resetted.emit(); // Notificar que se realizó el reset
      }, 0);
    }
  }
}
